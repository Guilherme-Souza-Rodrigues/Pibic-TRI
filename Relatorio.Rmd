...
<p align="center">
![Fluxo das menções.Podemos observar a evolução das menções atribuídas ao longo do curso, sendo que, na prova 1, há apenas a menção da própria prova; na prova 2, fez-se a média aritmética das duas primeiras notas; na prova 3, calculou-se a nota final de acordo com os pesos atribuídos a cada avaliação e na prova 4, vemos a nota final obtida pelo aluno, após a prova substitutiva.](sankey.png)
</p>



```{r, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE,fig.pos="H"}
if (!require("pacman")) install.packages("pacman")
...
```{r , echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap='Probabilidades de  que um aluno mediano passe', fig.subcap=c('Primeiro', 'Segundo'), out.width='.49\\linewidth', fig.asp=1, fig.ncol=2, fig.nrow=1,fig.pos="H"}
if (!require("pacman")) install.packages("pacman")
pacman::p_load("tidyverse", "dplyr", "tidyr", "reshape2", "irtoys", "ltm", "mirt", "bairt","ggplot2", "R.utils", "igraph", "factoextra", "threejs", "GGally")
...
```{r , echo=FALSE, fig.cap='(a) Correlograma dos temas. (b) Esta rede foi obtida a partir do cálculo dos resíduos feito para cada questão feita por cada aluno. O gráfico mostra a associação dos resíduos de cada tema. (c) Esta matriz de confusão compara as menções reais obtidas com as menções que seriam atribuídas por TRI. Espera-se que haja uma concentração maior na diagonal.', fig.subcap=c('Correlograma dos Temas', 'Rede de associação entre os Temas', 'Matriz de Confusão relacionando as notas reais com as notas por TRI'), out.width='.49\\linewidth', fig.asp=1, fig.ncol=2, fig.nrow=2,,fig.pos="H"}
library('igraph')
if (!require("pacman")) install.packages("pacman")
pacman::p_load("tidyverse", "dplyr", "tidyr", "reshape2", "irtoys", "ltm", "mirt", "bairt","ggplot2", "R.utils", "igraph", "factoextra", "threejs", "GGally")
...
```{r , echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, fig.cap='Probabilida de um aluno mediano (habilidade igual a zero) responder corretamente a questão segundo prova e tema',fig.height=9,fig.width=7,fig.pos="H"}
library('igraph')
if (!require("pacman")) install.packages("pacman")
...
```{r , echo=FALSE, warning=FALSE, message=FALSE,fig.cap="Curva caracteristica do item para cada item da primeira avaliação segundo tema " ,error=FALSE,fig.height=9,fig.width=7}
library('igraph')
if (!require("pacman")) install.packages("pacman")
pacman::p_load("tidyverse", "dplyr", "tidyr", "reshape2", "irtoys", "ltm", "mirt", "bairt","ggplot2", "R.utils", "igraph", "factoextra", "threejs", "GGally","knitr","kableExtra")
...
```{r , echo=FALSE, warning=FALSE, message=FALSE,fig.cap="Curva caracteristica do item para cada item da segunda avaliação segundo tema ", error=FALSE,fig.height=9,fig.width=7}
cci_p2()
```

```{r , echo=FALSE, warning=FALSE, message=FALSE,fig.cap="Curva caracteristica do item para cada item da terceira avaliação segundo tema ", error=FALSE,fig.height=9,fig.width=7}
cci_p3()
```


```{r , echo=FALSE, warning=FALSE, message=FALSE,fig.cap="Curva de informação do item para cada item da primeira avaliação segundo tema ", error=FALSE,fig.height=9,fig.width=7,fig.pos="H"}
library('igraph')
if (!require("pacman")) install.packages("pacman")
pacman::p_load("tidyverse", "dplyr", "tidyr", "reshape2", "irtoys", "ltm", "mirt", "bairt","ggplot2", "R.utils", "igraph", "factoextra", "threejs", "GGally","knitr","kableExtra")
...
```{r , echo=FALSE, warning=FALSE, message=FALSE,fig.cap="Curva de informação do item para cada item da terceira avaliação segundo tema ", error=FALSE,fig.height=9,fig.width=7}
fii_p2()
```
...
```{r , echo=FALSE, warning=FALSE, message=FALSE,fig.cap="Curva de informação do teste segunda avaliação e turma.", error=FALSE,fig.height=9,fig.width=7}
teste.inf()
```

```{r , echo=FALSE, warning=FALSE, message=FALSE, error=FALSE,fig.height=9,fig.width=7,fig.pos="H"}
library('igraph')
if (!require("pacman")) install.packages("pacman")
pacman::p_load("tidyverse", "dplyr", "tidyr", "reshape2", "irtoys", "ltm", "mirt", "bairt","ggplot2", "R.utils", "igraph", "factoextra", "threejs", "GGally","knitr","kableExtra")

kable(itens_p1,"latex",longtable = T,booktabs=F,caption = "Parâmetros para cada questão para a prova 1",align = c("l","l",rep("c",4)),digits = 3)%>%
           kable_styling(full_width=F,position="center",latex_options = c("repeat_header"))

kable(itens_p2,"latex",longtable = T,booktabs=F,caption = "Parâmetros para cada questão para a prova 2",align = c("l","l",rep("c",4)),digits = 3)%>%
           kable_styling(full_width=F,position="center",latex_options = c("repeat_header"))

kable(itens_p3,"latex",longtable = T,booktabs=F,caption = "Parâmetros para cada questão para a prova 3",align = c("l","l",rep("c",4)),digits = 3)%>%
           kable_styling(full_width=F,position="center",latex_options = c("repeat_header"))
